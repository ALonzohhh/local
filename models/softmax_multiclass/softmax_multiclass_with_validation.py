...
# ### 创建目录
if not os.path.exists('models/softmax_multiclass'):
    os.makedirs('models/softmax_multiclass')

if not os.path.exists('visualizations/softmax_multiclass'):
    os.makedirs('visualizations/softmax_multiclass')
...
with tqdm(total=100, desc="编码标签") as pbar:
    ...
    # 保存标签编码器
    joblib.dump(label_encoder, 'models/softmax_multiclass/label_encoder.joblib')
    
    # 保存类别映射关系
    class_mapping = {i: label for i, label in enumerate(label_encoder.classes_)}
    with open('models/softmax_multiclass/class_mapping.txt', 'w') as f:
...
def train_and_evaluate():
    # 初始化模型（Softmax回归就是多分类的逻辑回归）
    print("\n初始化Softmax回归模型...")
    model = LogisticRegression(
        multi_class='multinomial',  # 多项式Softmax回归
        solver='lbfgs',             # 求解器
        C=1.0,                      # 正则化强度的倒数
    ...
    # 保存模型
    joblib.dump(model, 'models/softmax_multiclass/softmax_model_with_validation.joblib')
    print("模型已保存")
    ...
    plt.xlabel('特征重要性')
    plt.title('Softmax回归模型 - 前20个重要特征')
    plt.tight_layout()
    plt.savefig('visualizations/softmax_multiclass/feature_importance_with_validation.png')
    ...
    with open('models/softmax_multiclass/validation_metrics.txt', 'w') as f:
    ...
    with open('models/softmax_multiclass/validation_classification_report.txt', 'w') as f:
    ...
    plt.title('Softmax回归多分类模型验证集混淆矩阵')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig('visualizations/softmax_multiclass/validation_confusion_matrix.png')
    ...
    plt.tight_layout()
    plt.savefig('visualizations/softmax_multiclass/validation_category_f1_scores.png')
    
    print("\nSoftmax回归多分类模型训练和评估完成！")
    print("可视化结果已保存到 'visualizations/softmax_multiclass/' 目录")
    print("\n总结:")
