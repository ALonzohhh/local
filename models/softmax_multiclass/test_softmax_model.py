...
def load_and_preprocess_data(test_data_path):
    ...
    # 加载训练好的标签编码器
    label_encoder_path = os.path.join('models', 'softmax_multiclass', 'label_encoder.joblib')
    print(f"加载标签编码器: {label_encoder_path}")
    label_encoder = joblib.load(label_encoder_path)
    
    # 加载类别映射
    class_mapping_path = os.path.join('models', 'softmax_multiclass', 'class_mapping.txt')
    class_mapping = {}
...
def test_model(model, X_test, y_test, class_mapping):
    ...
    with open('models/softmax_multiclass/test_metrics.txt', 'w') as f:
    ...
    with open('models/softmax_multiclass/test_report.txt', 'w') as f:
        f.write(report)
    ...
    plt.title('Softmax回归多分类模型测试混淆矩阵')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig('visualizations/softmax_multiclass/test_confusion_matrix.png')
    ...
    plt.tight_layout()
    plt.savefig('visualizations/softmax_multiclass/test_category_f1_scores.png')
    ...
    if hasattr(model, "predict_proba"):
        ...
        plt.xlabel('假正例率')
        plt.ylabel('真正例率')
        plt.title('Softmax回归多分类模型 - ROC曲线 (测试集)')
        plt.legend(loc="lower right")
        plt.tight_layout()
        plt.savefig('visualizations/softmax_multiclass/test_roc_curve.png')
...
def main():
    ...
    model_path = 'models/softmax_multiclass/softmax_model_with_validation.joblib'
