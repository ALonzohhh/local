...
# ### 创建目录
if not os.path.exists('models/svm_multiclass'):
    os.makedirs('models/svm_multiclass')

if not os.path.exists('visualizations/svm_multiclass'):
    os.makedirs('visualizations/svm_multiclass')
...
with tqdm(total=100, desc="编码标签") as pbar:
    ...
    # 保存标签编码器
    joblib.dump(label_encoder, 'models/svm_multiclass/label_encoder.joblib')
    
    # 保存类别映射关系
    class_mapping = {i: label for i, label in enumerate(label_encoder.classes_)}
    with open('models/svm_multiclass/class_mapping.txt', 'w') as f:
...
# 由于SVM训练时间较长，对大数据集可能需要采样
# 这里我们设置一个阈值，如果训练样本超过阈值，则采样
MAX_SAMPLES = 30000  # 最大样本数
USE_SAMPLE = len(X_train) > MAX_SAMPLES

if USE_SAMPLE:
    print(f"\n训练集样本数({len(X_train)})超过阈值({MAX_SAMPLES})，进行采样...")
    # 采样时保持类别比例
    from sklearn.model_selection import train_test_split
    X_train_sample, _, y_train_encoded_sample, _ = train_test_split(
        X_train, y_train_encoded, 
        train_size=MAX_SAMPLES,
        stratify=y_train_encoded,  # 保持类别比例
        random_state=42
    )
    print(f"采样后训练集大小: {X_train_sample.shape}")
    
    # 转换为numpy数组
    X_train_np = X_train_sample.values.astype(np.float32)
    y_train_encoded_np = y_train_encoded_sample
else:
    # 转换为numpy数组
...
def train_and_evaluate():
    ...
    # 初始化模型
    print(f"\n初始化SVM模型 (kernel={kernel}, C={C}, gamma={gamma})...")
    model = SVC(
        kernel=kernel,
        C=C,
        gamma=gamma,
    ...
    # 保存模型
    joblib.dump(model, 'models/svm_multiclass/svm_model_with_validation.joblib')
    print("模型已保存")
    ...
    with open('models/svm_multiclass/validation_metrics.txt', 'w') as f:
    ...
    with open('models/svm_multiclass/validation_classification_report.txt', 'w') as f:
    ...
    plt.title('SVM多分类模型验证集混淆矩阵')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig('visualizations/svm_multiclass/validation_confusion_matrix.png')
    ...
    plt.tight_layout()
    plt.savefig('visualizations/svm_multiclass/validation_category_f1_scores.png')
    
    print("\nSVM多分类模型训练和评估完成！")
    print("可视化结果已保存到 'visualizations/svm_multiclass/' 目录")
    print("\n总结:")
    if USE_SAMPLE:
        original_size = len(X_train)
        sample_size = len(X_train_np)
        print(f"\n注意: 由于原始训练集较大 ({original_size} 样本)，")
        print(f"模型只使用了采样后的数据集 ({sample_size} 样本, {sample_size/original_size*100:.2f}%)")
    print(f"- 使用{'采样后的' if USE_SAMPLE else '完整'}训练集: {len(X_train_np)}条记录")
